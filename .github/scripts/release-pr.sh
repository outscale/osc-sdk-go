...
result=$(curl -s -X POST -u LolUserName:$GITHUB_TOKEN -d "{\"head\":\"$branch_name\",\"base\":\"v$major\",\"title\":\"$new_sdk_version version\",\"body\":\"Automatic build of $new_sdk_version version based on $osc_api_version\"}" "https://api.github.com/repos/outscale/osc-sdk-go/pulls")
