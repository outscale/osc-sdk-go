...
type ApiReadVmsStopHistoryRequest struct {
	ctx                       _context.Context
	ApiService                *VmApiService
	readVmsStopHistoryRequest *ReadVmsStopHistoryRequest
}

func (r ApiReadVmsStopHistoryRequest) ReadVmsStopHistoryRequest(readVmsStopHistoryRequest ReadVmsStopHistoryRequest) ApiReadVmsStopHistoryRequest {
	r.readVmsStopHistoryRequest = &readVmsStopHistoryRequest
	return r
}

func (r ApiReadVmsStopHistoryRequest) Execute() (ReadVmsStopHistoryResponse, *_nethttp.Response, error) {
	return r.ApiService.ReadVmsStopHistoryExecute(r)
}

/*
 * ReadVmsStopHistory Method for ReadVmsStopHistory
 * Lists the stop history of one or more VMs.
 * @param ctx _context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @return ApiReadVmsStopHistoryRequest
 */
func (a *VmApiService) ReadVmsStopHistory(ctx _context.Context) ApiReadVmsStopHistoryRequest {
	return ApiReadVmsStopHistoryRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
/*
 * Execute executes the request
 * @return ReadVmsStopHistoryResponse
 */
func (a *VmApiService) ReadVmsStopHistoryExecute(r ApiReadVmsStopHistoryRequest) (ReadVmsStopHistoryResponse, *_nethttp.Response, error) {
	var (
		localVarHTTPMethod   = _nethttp.MethodPost
		localVarPostBody     interface{}
		localVarFormFileName string
		localVarFileName     string
		localVarFileBytes    []byte
		localVarReturnValue  ReadVmsStopHistoryResponse
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "VmApiService.ReadVmsStopHistory")
	if err != nil {
		return localVarReturnValue, nil, GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/ReadVmsStopHistory"
	...
	// body params
	localVarPostBody = r.readVmsStopHistoryRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["ApiKeyAuth"]; ok {
